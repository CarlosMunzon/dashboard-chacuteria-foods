...
def filter_df(df, prefix):
    ...
    if clientes: out = out[out["cliente"].isin(clientes)]
    if categorias: out = out[out["categoria"].isin(categorias)]
    if regionales: out = out[out["regional"].isin(regionales)]
    return out
...
si_m = si_f[si_f["sku_valido"]==True].assign(mes=lambda d: d["fecha"].values.astype("datetime64[M]")).groupby("mes", as_index=False).agg(sell_in_valor=("valor","sum"), sell_in_kilos=("kilos","sum"))
so_m = so_f[so_f["sku_valido"]==True].assign(mes=lambda d: d["fecha"].values.astype("datetime64[M]")).groupby("mes", as_index=False).agg(sell_out_valor=("valor","sum"), sell_out_kilos=("kilos","sum"))
trend = pd.merge(si_m, so_m, on="mes", how="outer").fillna(0).sort_values("mes")
fig = go.Figure(); fig.add_trace(go.Scatter(x=trend["mes"], y=trend["sell_in_valor"]/1e9, mode="lines+markers", name="Sell-in")); fig.add_trace(go.Scatter(x=trend["mes"], y=trend["sell_out_valor"]/1e9, mode="lines+markers", name="Sell-out")); fig.update_layout(height=360, template="plotly_white", title="Evolución mensual valor (COP billones)")
...
st.dataframe(alerts_inv[["cliente","descripcion_producto","riesgo","inv_kilos_cierre","doh_30d"]].sort_values(["riesgo","doh_30d"]).head(50), use_container_width=True)
st.subheader("Mercado")
figm = go.Figure(); figm.add_trace(go.Scatter(x=market_sum["mes"], y=market_sum["share_compania_prom"]*100, mode="lines+markers", name="Compañía")); figm.add_trace(go.Scatter(x=market_sum["mes"], y=market_sum["share_competidor_1_prom"]*100, mode="lines+markers", name="Competidor 1")); figm.add_trace(go.Scatter(x=market_sum["mes"], y=market_sum["share_competidor_2_prom"]*100, mode="lines+markers", name="Competidor 2")); figm.update_layout(height=340, template="plotly_white", title="Share promedio mensual (%)")
st.plotly_chart(figm, use_container_width=True)
st.subheader("Alertas atípicas")
st.dataframe(alerts_beh[["anio_mes","cliente","categoria","valor","base_3m","var_vs_base_pct","alerta"]].sort_values("var_vs_base_pct").head(50), use_container_width=True)
